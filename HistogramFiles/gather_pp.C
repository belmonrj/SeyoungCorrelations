void gather_pp()
{
  ...
  TFile* file_CNT_BBCN = TFile::Open("pp_CorHisto_CNT_BBCN.root");
  TFile* file_CNT_BBCS = TFile::Open("pp_CorHisto_CNT_BBCS.root");
  TFile* file_CNT_FVTN = TFile::Open("pp_CorHisto_CNT_FVTN.root");
  TFile* file_CNT_FVTS = TFile::Open("pp_CorHisto_CNT_FVTS.root");

  TFile* file_BBCN_BBCS = TFile::Open("pp_CorHisto_BBCN_BBCS.root");
  TFile* file_BBCN_FVTN = TFile::Open("pp_CorHisto_BBCN_FVTN.root");
  TFile* file_BBCN_FVTS = TFile::Open("pp_CorHisto_BBCN_FVTS.root");

  TFile* file_FVTN_BBCS = TFile::Open("pp_CorHisto_FVTN_BBCS.root");
  TFile* file_FVTN_FVTS = TFile::Open("pp_CorHisto_FVTN_FVTS.root");
  TFile* file_FVTS_BBCS = TFile::Open("pp_CorHisto_FVTS_BBCS.root");
  ...
  TFile* fSeyoung = new TFile("seyoung_data_pp.root","recreate");

  for ( int imult = 0; imult < nmultbins; ++imult )
    {
      // ---
      h_BBCN_BBCS[imult] = (TH1D*)file_BBCN_BBCS->Get(Form("COR_BBCN_BBCS_C%d",imult));
      h_BBCN_FVTN[imult] = (TH1D*)file_BBCN_FVTN->Get(Form("COR_BBC_FVTN_C%d",imult));
      h_BBCN_FVTS[imult] = (TH1D*)file_BBCN_FVTS->Get(Form("COR_BBCN_FVTS_C%d",imult));
      ...
      // ---
      h_FVTN_BBCS[imult] = (TH1D*)file_FVTN_BBCS->Get(Form("COR_BBCS_FVTN_C%d",imult));
      h_FVTN_FVTS[imult] = (TH1D*)file_FVTN_FVTS->Get(Form("COR_FVTN_FVTS_C%d",imult));
      h_FVTS_BBCS[imult] = (TH1D*)file_FVTS_BBCS->Get(Form("COR_BBC_FVTS_C%d",imult));
      ...
      for ( int ipt = 0; ipt < nptbins; ++ipt )
        {
          // ---
          h_CNT_BBCN[imult][ipt] = (TH1D*)file_CNT_BBCN->Get(Form("COR_CNT_BBCN_C%d_pT%d",imult,ipt));
          if ( h_CNT_BBCN[imult][ipt] == NULL ) cout << "uh oh CNT_BBCN" << endl;
          // ---
          h_CNT_BBCS[imult][ipt] = (TH1D*)file_CNT_BBCS->Get(Form("COR_CNT_BBCS_C%d_pT%d",imult,ipt));
          if ( h_CNT_BBCS[imult][ipt] == NULL ) cout << "uh oh CNT_BBCS" << endl;
          // ---
          h_CNT_FVTN[imult][ipt] = (TH1D*)file_CNT_FVTN->Get(Form("COR_CNT_FVTN_C%d_pT%d",imult,ipt));
          if ( h_CNT_FVTN[imult][ipt] == NULL ) cout << "uh oh CNT_FVTN" << endl;
          // ---
          h_CNT_FVTS[imult][ipt] = (TH1D*)file_CNT_FVTS->Get(Form("COR_CNT_FVTS_C%d_pT%d",imult,ipt));
